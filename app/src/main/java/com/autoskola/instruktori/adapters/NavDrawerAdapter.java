...
public class NavDrawerAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		LinearLayout container = (LinearLayout) convertView.findViewById(R.id.counterContainer);
		RelativeLayout textcont = (RelativeLayout) convertView.findViewById(R.id.textcont);
		TextView counter = (TextView) convertView.findViewById(R.id.counter);

		
		imgIcon.setImageResource(navDrawerItems.get(position).getIcon());
		txtTitle.setText(navDrawerItems.get(position).getTitle());
		if(navDrawerItems.get(position).getCount().isEmpty()){
